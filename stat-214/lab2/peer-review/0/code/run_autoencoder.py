...
def main():
    ...
    # get patches for individual images
    patched_images = [image_patches for image_patches in patches]
    # randomly do train/val split within images

    print("random split for each image")
    train_patches = []
    val_patches = []
    for image_patches in patched_images:
        train_bool = np.random.rand(len(image_patches)) < 0.8
        train_idx = np.where(train_bool)[0]
        val_idx = np.where(~train_bool)[0]

        # create train and val datasets
        train = [image_patches[i] for i in train_idx]
        val = [image_patches[i] for i in val_idx]
        train_patches.extend(train)
        val_patches.extend(val)
